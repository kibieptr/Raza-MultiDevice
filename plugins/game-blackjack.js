async function handler(m, { conn, usedPrefix, command, text }) {
  ...
  if (m.sender in conn.bj)
    return m.reply("Kamu masih dalam game, tunggu sampai selesai!!");
  try {
    let cards = [
      "A",
      "2",
    ...
    if (!(m.sender in conn.bj)) {
      ...
      if (playerTotal === 21 && computerTotal !== 21) {
        sendResultMessage(
          `*Kamu mendapatkan Blackjack! Kamu menang!*\n*+${bet} Balance*`,
          playerTotal,
          computerTotal
        );
        ...
        sendResultMessage(
          "*Hasilnya SERI! Keduanya mendapatkan Blackjack!*",
          playerTotal,
          computerTotal
        );
        delete conn.bj[m.sender];
...
handler.before = async (m) => {
  ...
  try {
    if (/^hit?$/i.test(txt)) {
      ...
      if (playerTotal > 21) {
        sendResultMessage(
          `*Kamu kalah! Total kartu melebihi 21.*\n*-${bet} Balance*`,
          playerTotal,
          computerTotal
        );
      ...
      if (computerTotal > 21) {
        sendResultMessage(
          `*Kamu menang! Total kartu Komputer melebihi 21.*\n*+${bet} Balance*`,
          playerTotal,
          computerTotal
        );
        global.db.data.users[m.sender].money += bet * 1;
