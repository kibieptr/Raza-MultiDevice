...
let handler = async (m, { conn, usedPrefix, text }) => {
  ...
  if (apelu == 0 || angguru == 0 || manggau == 0 || pisangu == 0 || jeruku == 0)
    return m.reply(
      `*Pastikan Kamu Memiliki Semua Bibit*\n*Seperti Bibit Apel, Bibit Mangga, Bibit Jeruk, Bibit Pisang, Bibit Anggur Masing Masing 100*\n\nKetik :\n${usedPrefix}shop buy bibitmangga 500\n\n*List*\nbibitmangga\nbibitanggur\nbibitpisang\nbibitjeruk\nbibitapel`
    );
  ...
  if (global.db.data.users[m.sender].bibitmangga > 100) {
    if (global.db.data.users[m.sender].bibitapel > 100) {
      if (global.db.data.users[m.sender].bibitpisang > 100) {
        if (global.db.data.users[m.sender].bibitjeruk > 100) {
          if (global.db.data.users[m.sender].bibitanggur > 100) {
            ...
            m.reply(`Pastikan Bibit Anggur Kamu *100* Untuk Bisa Berkebun`);
