let handler = async (m, { conn, usedPrefix, DevMode }) => {
  try {
    ...
    if (global.db.data.users[m.sender].healt > 79) {
      if (new Date() - global.db.data.users[m.sender].lastnebang > 180000) {
...
`.trim();
        ...
        conn.reply(m.chat, `Kamu Sudah Menebang, Tunggu Selama ${timers}`, m);
    } else
      conn.reply(
        m.chat,
        "Minimal 80 Health Untuk Menebang\n*Ketik: .heal* Untuk Menggunakan Potion",
        m
      );
  } catch (e) {
    console.log(e);
