...
class SnakeAndLadderGame {
  ...
  startGame = async (m, player1Name, player2Name) => {
    await m.reply(
      `🐍🎲 *Selamat datang di Permainan Ular Tangga!* 🎲🐍 \n\n${this.formatPlayerName(
        player1Name
      )} *vs* ${this.formatPlayerName(player2Name)}`,
      null,
      {
  ...
  playTurn = async (m, player) => {
    ...
    if (player !== this.players[this.currentPlayerIndex])
      return m.reply(
        `🕒 *Bukan giliranmu.* \n\nSekarang giliran ${this.formatPlayerName(
    ...
    for (const otherPlayer of this.players) {
      if (
        ...
        const message = `😱 *Oh tidak!* ${this.formatPlayerName(
          player
        )} *menginjak posisi* ${this.formatPlayerName(
          otherPlayer
    ...
    if (newPosition <= this.boardSize) {
      ...
      if (checkSnakeOrLadder) {
        ...
        await m.reply(
          `🐍 ${this.formatPlayerName(player)} menemukan *${
    ...
    case "roll":
      ...
      if (game.isGameStarted()) {
        ...
        if (m.sender !== currentPlayer) {
          await m.reply(
            `🕒 *Bukan giliranmu.* \n\nSekarang giliran ${game.formatPlayerName(
              currentPlayer
            )}`,
            null,
            {
